...
const scrapeAnimeByGenre = (html: string) => {
  ...
  const animeElements = $(".venser .page .col-anime-con")
    .toString()
    .split(
      '<div class="col-md-4 col-anime-con genre_2 genre_3 genre_4 genre_9 ">'
    )
    .filter((element) => element.trim() !== "")
    .map(
      (element) =>
        `<div class="col-md-4 col-anime-con genre_2 genre_3 genre_4 genre_9 ">${element}`
    );
  ...
  animeElements.forEach((animeEl) => {
    const $ = load(animeEl);
    const episodeCount = $(".col-anime .col-anime-eps")
      .text()
      .replace(/[A-z]/g, "")
      .trim();
    ...
    result.push({
      ...
      slug: $(".col-anime .col-anime-trailer a")
        .attr("href")
