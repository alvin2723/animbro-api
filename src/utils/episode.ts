...
const episode = async ({
  ...
  if (animeSlug && episodeNumber) {
    ...
    const splittedEpisodeSlug = episodeLists[0].slug?.split(
      "-episode-"
    ) as string[];
    const prefixEpisodeSlug = splittedEpisodeSlug[0];
    const firstEpisodeNumber = splittedEpisodeSlug[1].replace("-sub-indo", "");

    if (episodeNumber.toString() === "special") {
      slug = `${prefixEpisodeSlug}-episode-special-sub-indo`;
    } else {
      slug = `${prefixEpisodeSlug}-episode-${
        episodeNumber - (parseInt(firstEpisodeNumber) == 0 ? 1 : 0)
      }-sub-indo`;
    }
  }
